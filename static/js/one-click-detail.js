(function(){
    ...
    const DETAIL_STORAGE_KEY = canvasNodeId ? `studio_one_click_detail_canvas_${canvasNodeId}_draft_v1` : 'studio_one_click_detail_draft_v3';
    const LEGACY_STORAGE_KEY = canvasNodeId ? '' : 'studio_one_click_detail_draft_v2';
    const DETAIL_PRESET_KEY = 'studio_one_click_detail_preset_v3';
    const LEGACY_PRESET_KEY = 'studio_one_click_detail_preset_v2';
    ...
    if(window.StudioI18n){
        StudioI18n.register({
            'detail.ready':{zh:'详情页工作台',en:'Detail page workspace'},
            'detail.title':{zh:'一键详情',en:'One-click Detail Page'},
            'detail.subtitle':{zh:'使用当前自由接口生成连贯的电商详情页分段。',en:'Generate coherent ecommerce detail-page sections with the configured APIs.'},
    ...
    function applyLanguage(){
        ...
        document.title = !title || title === 'detail.title' ? '一键详情' : title;
    }
    ...
    function modelLikelySupportsReferenceImages(model){
        const value = String(model || '').trim().toLowerCase();
        if(!value) return false;
        return /edit|image[-_/.]?to[-_/.]?image|img2img|i2i|kontext|inpaint|outpaint|gpt-image|gemini|nano[-_/.]?banana|flux[^/]*klein/.test(value);
    }

    function imageModelForSources(providerId, requestedModel, sources){
        const provider = state.providers.find(item => item.id === providerId);
        const models = providerModels(provider);
        let model = models.includes(requestedModel) ? requestedModel : (models[0] || requestedModel || '');
        const needsReferenceSupport = (Array.isArray(sources) ? sources : []).some(item => item?.url);
        if(needsReferenceSupport && models.length && !modelLikelySupportsReferenceImages(model)){
            model = models.find(modelLikelySupportsReferenceImages) || model;
        }
        return model;
    }
    ...
    function modelLikelySupportsImages(model){
        const value = String(model || '').trim().toLowerCase();
        if(!value) return false;
        return /(^|[-_/.])(vl|vision)([-_/.]|$)|qwen[^/]*vl|gpt-4o|gpt-4\.1|gpt-5|gemini|claude-(?:3|sonnet|opus)|doubao[^/]*vision|kimi[^/]*vision|minimax[^/]*vl/.test(value);
    }
    ...
    function sourceRoleManifest(sources){
        const items = (Array.isArray(sources) ? sources : [])
            .filter(item => item?.url)
            .slice(0, MAX_SOURCE_IMAGES);
        if(!items.length) return '';
        const roles = items.map((item, index) => {
            const role = item.role === 'product' ? '产品图' : '风格参考图';
            const name = String(item.name || '').replace(/\s+/g, ' ').trim();
            return `第${index + 1}张=${role}${name ? `（${name.slice(0, 40)}）` : ''}`;
        });
        return [
            `输入图片角色与顺序：${roles.join('；')}。`,
            '产品图必须用于锁定真实商品的外观、颜色、材质、结构、比例、Logo、数量以及产品图中指定模特的身份。',
            '风格参考图只允许参考配色、构图、字体气质和版式，不得复制其中的商品、品牌、文案、商标或人物。',
            '必须严格按以上序号理解图片，不得交换产品图与风格参考图的用途。'
        ].join('\n');
    }

    function withSourceRoleManifest(prompt, sources){
        const manifest = sourceRoleManifest(sources);
        return manifest ? `${String(prompt || '').trim()}\n\n${manifest}` : String(prompt || '').trim();
    }
    ...
    function syncSizeOptions(preferred=''){
        const values = {
            portrait916:['720x1280','1152x2048','2160x3840'],
        ...
        }[$('ratioSelect').value] || ['720x1280','1152x2048','2160x3840'];
    ...
    function directiveNumber(pattern, fallback){
        const match = String($('userPrompt')?.value || '').match(pattern);
        return match ? Number(match[1]) : fallback;
    }

    function selectedCount(){
        return clampNumber($('countSelect')?.value, 1, 12, 8);
    }

    function effectiveCount(){
        return clampNumber(directiveNumber(/输出\s*(\d+)\s*屏/i, selectedCount()), 1, 30, selectedCount());
    }

    function effectiveReverseScreens(count=effectiveCount()){
        const selected = clampNumber($('reverseScreens')?.value, 0, count, Math.min(2, count));
        return clampNumber(directiveNumber(/插入\s*反转屏\s*(\d+)\s*屏/i, selected), 0, count, selected);
    }

    function replaceDirectivePrefix(count, reverseScreens){
        const textarea = $('userPrompt');
        if(!textarea) return;
        const prefix = `输出${count}屏，插入反转屏${reverseScreens}屏。`;
        const rest = textarea.value.replace(/^\s*输出\s*\d+\s*屏\s*[，,、]?\s*插入\s*反转屏\s*\d+\s*屏\s*[。.!！]?\s*/i, '').trimStart();
        textarea.value = rest ? `${prefix}\n${rest}` : prefix;
    }

    function rebuildNumberOptions(id, max, preferred){
        const select = $(id);
        if(!select) return;
        const value = clampNumber(preferred ?? select.value, 0, max, 0);
        if(select.options.length !== max + 1 || Number(select.options[select.options.length - 1]?.value) !== max){
            select.innerHTML = Array.from({length:max + 1}, (_, index) => `<option value="${index}">${index}</option>`).join('');
        }
        select.value = String(value);
    }

    function syncCountDependentControls(preferred={}){
        const count = clampNumber(preferred.count ?? effectiveCount(), 1, 30, 8);
        rebuildNumberOptions('modelUsage', count, preferred.modelUsage ?? $('modelUsage')?.value ?? Math.min(4, count));
        rebuildNumberOptions('reverseScreens', count, preferred.reverseScreens ?? $('reverseScreens')?.value ?? Math.min(2, count));
        const disabled = $('modelSettings')?.value === 'none';
        if($('modelUsage')) $('modelUsage').disabled = disabled;
        if($('modelPose')) $('modelPose').disabled = disabled;
    }

    function handleCountChange(){
        const count = selectedCount();
        const reverseScreens = clampNumber($('reverseScreens')?.value, 0, count, Math.min(2, count));
        syncCountDependentControls({count, reverseScreens});
        replaceDirectivePrefix(count, effectiveReverseScreens(count));
        scheduleDraftSave();
    }

    function handleReverseScreensChange(){
        const count = effectiveCount();
        replaceDirectivePrefix(count, clampNumber($('reverseScreens')?.value, 0, count, 0));
        scheduleDraftSave();
    }

    function handleUserPromptInput(){
        const count = effectiveCount();
        syncCountDependentControls({count, reverseScreens:effectiveReverseScreens(count)});
        scheduleDraftSave();
    }
    ...
    function formValues(){
        ...
        return {
            ...
            count,
            reverseScreens:effectiveReverseScreens(count),
            ...
            modelUsage:$('modelSettings').value === 'none' ? 0 : clampNumber($('modelUsage').value, 0, count, Math.min(4, count)),
    ...
    function copyRule(config){
        ...
        if(config.copySetting === 'poster') return '生成无文案纯海报，不出现任何文字、字母、数字或文案留白框，让画面作为纯视觉海报使用。';
        return `使用${config.outputLanguage}生成少量准确、易读的电商文案，主标题简短，卖点清晰。`;
    }

    function fontRule(config){
        const style = {
            auto:'结合商品特性与参考图自动判断最适合的字体风格',
            'neutral-sans':'现代中性无衬线',
            ...
            industrial:'工业力量字体',
            'trend-hand':'潮流手写展示字体'
        }[config.fontStyle] || '自动判断';
        return `全组字体风格固定为${style}。AI 可以做轻微字形变体，但所有屏的字面气质、字重层级、标题结构、描边、阴影、颜色和排版节奏必须一致；文字必须清晰、正确、可读，避免乱码、错字和过密小字。`;
    }

    function spreadIndexes(total, count, edge=false){
        const result = new Set();
        const amount = clampNumber(count, 0, total, 0);
        if(amount <= 0) return result;
        for(let index = 0; index < amount; index++){
            const value = edge && amount > 1
                ? Math.round(index * (total - 1) / (amount - 1))
                : Math.max(0, Math.min(total - 1, Math.round((index + 1) * total / (amount + 1)) - 1));
            result.add(value);
        }
        for(let index = 0; result.size < amount && index < total; index++) result.add(index);
        return result;
    }

    function modelRule(config, index){
        if(config.modelSettings === 'none') return '不要使用模特或人物。';
        if(!spreadIndexes(config.count, config.modelUsage, true).has(index)) return '本屏不使用模特，让商品独立成为主体。';
        const pose = config.modelPose === 'special' ? '具有设计感的特殊姿态' : '自然、常规、可信的展示姿态';
        return `本屏必须使用与商品定位匹配的模特，采用${pose}，模特不能遮挡商品关键结构；若产品图中包含指定模特，必须保持同一人物身份与外观。`;
    }

    function reverseRule(config, index){
        if(!spreadIndexes(config.count, config.reverseScreens).has(index)) return '本屏延续全组主色背景。';
        return '本屏为反转屏：在保持同一视觉系统、字体、强调色和商品质感的前提下，反转背景明暗或主辅色关系，用于详情页节奏切换。';
    }

    function localPromptCards(config){
        const plans = [
            ['首屏核心主视觉', '建立商品认知，突出产品名称、第一核心利益点和完整商品形象'],
            ['需求与痛点', '用真实使用问题或用户需求引出商品价值，避免夸张和贬低竞品'],
            ['核心功能', '聚焦最重要的功能价值，用清晰的视觉证据解释为什么有效'],
            ['结构与细节', '用近景、剖面或局部特写表现结构、材质、工艺与品质'],
            ['使用方式', '展示操作步骤、握持方式、穿戴方式或使用前后的正确状态'],
            ['真实场景', '放入最典型的使用环境，表现适用人群、空间和生活方式'],
            ['对比与优势', '用可信的对比结构强化购买理由，保持信息克制且不出现竞品商标'],
            ['多场景延展', '展示不同时间、地点或人群下的使用场景，扩展购买想象'],
            ['规格参数', '用规整易读的信息布局承载尺寸、容量、材质、参数或注意事项'],
            ['品质与信任', '表现检测、工艺、包装、服务或可靠性，建立购买信任'],
            ['礼赠与人群', '展示礼赠、节日、家庭或朋友场景，说明适合的人群'],
            ['收束海报', '以完整商品和品牌感收束整套详情页，形成明确的购买行动感']
        ];
        const styleGuide = [
            '全组使用同一套商业视觉系统：固定主色、强调色、背景材质、光线方向、摄影质感、图形装饰和留白比例',
            fontRule(config),
            '所有屏上下拼接后要像同一个详情页，信息密度有疏有密但风格连续；禁止每屏重新发明一套设计',
            '参考图只参考风格、配色、构图、字体颜色与版式结构，不复制参考图中的商品、品牌、文案、商标或模特'
        ].join('。');
        return Array.from({length:config.count}, (_, index) => {
            ...
            const prompt = [
                `这是完整电商详情页的第${index + 1}屏，共${config.count}屏。所有屏必须保持同一商品、同一视觉系统和明确的信息顺序，本屏承担独立的信息任务。`,
                `全组视觉规范：${styleGuide}。`,
                ...
                reverseRule(config, index),
                `输出比例：${config.ratioLabel}，分辨率目标：${config.size}。`,
                '严格保留所有产品图中的真实外观、颜色、材质、结构、比例、Logo位置和数量，不得擅自改款，不得添加无关商品。商业摄影与平面设计结合，边缘干净，主体完整，适合淘宝、天猫或跨境电商详情页直接使用。'
            ].filter(Boolean).join('');
            return {title, prompt};
        });
    }

    function chatTarget(preferred={}, referenceImages=[]){
        ...
        let model = models.includes(requestedModel) ? requestedModel : (models[0] || requestedModel || '');
        const needsVision = referenceImages.some(item => item?.url);
        if(needsVision && models.length && !modelLikelySupportsImages(model)){
            model = models.find(modelLikelySupportsImages) || model;
        }
        return {provider:provider.id, model};
    }

    async function requestChat(message, systemPrompt, referenceImages=[], signal, preferredTarget={}){
        const target = chatTarget(preferredTarget, referenceImages);
    ...
    function parsePromptCards(raw, count){
        ...
        const cards = Array.isArray(source) ? source.map((item, index) => {
            if(typeof item === 'string') return {title:`第 ${index + 1} 屏`, prompt:item.trim()};
            return {title:String(item?.title || item?.name || `第 ${index + 1} 屏`).trim(), prompt:String(item?.prompt || item?.content || '').trim()};
        }).filter(item => item.prompt) : [];
        return {
            styleGuide:String(parsed?.style_guide || parsed?.styleGuide || '').trim(),
    ...
    async function analyzePrompts(group){
        ...
        const systemPrompt = [
            '你是资深电商详情页策划、视觉导演和分镜设计师。',
            `必须输出严格 JSON：{"style_guide":"整组统一视觉规范","prompts":[{"title":"第1屏短标题","prompt":"完整中文生图提示词"}]}，prompts 数组必须正好 ${group.config.count} 项。`,
            '不要输出 Markdown，不要解释，不要省略字段。',
            '用户指令拥有最高优先级，用户指令中的输出屏数可以覆盖下拉数量。',
            '整组是自上而下阅读的电商详情页：首屏建立认知，中段依次解释痛点、功能、结构、场景、参数和信任，末屏负责收束；每屏任务不同，但必须像同一位设计师完成。',
            `必须准确安排 ${group.config.reverseScreens} 个反转屏，并均匀安排 ${group.config.modelUsage} 个模特屏；不使用模特时严禁出现人物。`,
            'style_guide 必须固定整组主色、强调色、背景材质、光线方向、摄影质感、图形装饰、字体气质、标题层级、描边阴影、留白与信息密度。',
            '每条提示词都必须重复 style_guide、产品一致性、文案规则、字体、语言、模特、反转屏、比例、分辨率与参考图边界，保证单独提交图片模型时仍然完整。',
            '产品图决定真实商品外观、颜色、材质、结构、比例、Logo与指定模特身份；参考图只参考风格、配色、构图和版式，不得复制其中的商品、品牌、文案、商标或人物。'
        ].join('\n');
        const message = withSourceRoleManifest(
            `请为一套完整电商详情页生成分段提示词。输出比例固定为 ${group.config.ratioLabel}，分辨率 ${group.config.size}。配置如下：\n${JSON.stringify(group.config, null, 2)}`,
            group.sources
        );
        try {
            ...
            if(!parsed.cards.length) throw new Error('文本模型没有返回可解析的分段提示词');
            const merged = Array.from({length:group.config.count}, (_, index) => parsed.cards[index] || fallback[index]).map(item => ({
                ...item,
                prompt:parsed.styleGuide && !item.prompt.includes(parsed.styleGuide)
                    ? `全组统一视觉规范：${parsed.styleGuide}。${item.prompt}`
                    : item.prompt
            }));
            group.styleGuide = parsed.styleGuide;
    ...
    function createGroup(){
        ...
        const sources = selectedRefs().map(item => ({...item}));
        config.model = imageModelForSources(config.provider, config.model, sources);
        if(config.provider === state.provider && config.model && config.model !== state.model){
            state.model = config.model;
            $('modelSelect').value = config.model;
        }
        const serial = ++state.groupSerial;
        const productName = config.productName.replace(/\s+/g, ' ').trim();
        return {
            id:uid(),
            serial,
            title:productName ? `${productName}详情` : `详情组 #${serial}`,
            status:'analyzing',
            error:'',
            analysisSource:'',
            analysisNotice:'',
    ...
    function imageRequestBody(group, prompt, sources){
        const referenceImages = Array.isArray(sources) ? sources : (group.sources || []);
        return {
            prompt:withSourceRoleManifest(prompt, referenceImages),
            provider_id:group.config.provider || state.provider || 'comfly',
            model:imageModelForSources(group.config.provider || state.provider, group.config.model || '', referenceImages),
    ...
    async function runGroup(group){
        ...
        try {
            ...
            refreshGroups();
            syncGenerateButton();
            setStatus(`${group.title} 分析完成，开始生成 ${group.cards.length} 个详情屏。`);
    ...
    function cardMarkup(group, card, index){
        ...
        return `
            <div class="output-card ${esc(card.status)} prompt-${esc(promptStatus)} ${card.selected ? 'selected' : ''}" data-preview-card="${card.url ? 'true' : 'false'}" data-group-id="${esc(group.id)}" data-card-index="${index}">
                <div class="card-topline" draggable="true" data-card-drag data-group-id="${esc(group.id)}" data-card-index="${index}" aria-label="第 ${index + 1} 屏"><span>${String(index + 1).padStart(2,'0')}</span><i data-lucide="more-horizontal"></i></div>
    ...
    function groupMarkup(group){
        ...
        const body = group.status === 'analyzing'
            ? `<div class="group-analysis"><span class="detail-spinner"></span><strong>正在分析产品与参考图</strong><small>正在规划 ${targetCount} 个连续详情屏，完成后自动创建图片卡片。</small></div>`
    ...
    function collageSheet(){
        return $('collageGrid')?.querySelector('.collage-sheet') || null;
    }

    function applyCollageZoom(){
        const sheet = collageSheet();
        const label = $('collageZoomLabel');
        if(!sheet){
            if(label) label.textContent = state.collageFit ? '适应' : `${Math.round(state.collageZoom * 100)}%`;
            return;
        }
        sheet.classList.toggle('fit', state.collageFit);
        if(state.collageFit){
            sheet.style.removeProperty('width');
            if(label) label.textContent = '适应';
        } else {
            const naturalWidth = Math.max(1, Number(sheet.naturalWidth) || 2048);
            sheet.style.width = `${Math.round(naturalWidth * state.collageZoom)}px`;
            if(label) label.textContent = `${Math.round(state.collageZoom * 100)}%`;
        }
    }

    function setCollageZoom(value, fit=false){
        state.collageFit = Boolean(fit);
        state.collageZoom = Math.max(.25, Math.min(2, Math.round(Number(value || 1) * 4) / 4));
        applyCollageZoom();
    }

    function toggleCollageFit(){
        setCollageZoom(1, !state.collageFit);
    }

    function syncCollageFullscreenButton(){
        const button = $('collageFullscreen');
        if(!button) return;
        const active = document.fullscreenElement === document.querySelector('.collage-dialog');
        button.title = active ? '退出全屏' : '全屏';
        button.innerHTML = `<i data-lucide="${active ? 'minimize-2' : 'maximize-2'}"></i>`;
        window.lucide?.createIcons();
    }

    async function toggleCollageFullscreen(){
        const dialog = document.querySelector('.collage-dialog');
        if(!dialog) return;
        try {
            if(document.fullscreenElement) await document.exitFullscreen();
            else await dialog.requestFullscreen();
        } catch(error){
            setStatus('当前环境不支持全屏预览。', true);
        }
        syncCollageFullscreenButton();
    }
    ...
    async function openCollage(group){
        ...
        try {
            ...
            $('collageGrid').innerHTML = `<img class="collage-sheet fit" src="${esc(state.collageObjectUrl)}" alt="${esc(group.title || '完整详情长图')}">`;
            collageSheet()?.addEventListener('load', applyCollageZoom, {once:true});
            applyCollageZoom();
    ...
    function closeCollage(){
        if(document.fullscreenElement === document.querySelector('.collage-dialog')) void document.exitFullscreen();
    ...
    function downloadCardImage(group, index){
        const card = group?.cards?.[index];
        if(!card?.url) return;
        const link = document.createElement('a');
        link.href = card.url;
        link.download = `${group.title || `详情组-${group.serial}`}-${String(index + 1).padStart(2, '0')}.png`.replace(/[\\/:*?"<>|]+/g, '_');
        document.body.appendChild(link);
        link.click();
        link.remove();
    }
    ...
    function closeCardContextMenu(){
        ...
        state.cardMenuTarget = null;
    }

    function openCardContextMenu(event, group, index){
        const card = group?.cards?.[index];
        const menu = $('cardContextMenu');
        if(!card || !menu) return;
        event.preventDefault();
        state.cardMenuTarget = {groupId:group.id, cardIndex:index};
        const retry = menu.querySelector('[data-context-action="retry"]');
        const download = menu.querySelector('[data-context-action="download"]');
        if(retry) retry.disabled = hasGenerationActivity() || ['pending','running'].includes(card.status);
        if(download) download.disabled = !card.url;
        menu.hidden = false;
        menu.style.left = '0px';
        menu.style.top = '0px';
        const scale = Math.max(.01, Number(getComputedStyle(document.documentElement).getPropertyValue('--studio-ui-scale')) || 1);
        const viewportWidth = window.innerWidth / scale;
        const viewportHeight = window.innerHeight / scale;
        const left = Math.max(8, Math.min(event.clientX / scale, viewportWidth - menu.offsetWidth - 8));
        const top = Math.max(8, Math.min(event.clientY / scale, viewportHeight - menu.offsetHeight - 8));
    ...
    async function optimizePrompt(){
        ...
        refreshGroups();
        scheduleDraftSave();
        const systemPrompt = '你是电商详情页分镜提示词编辑器。严格遵守修改要求，同时保留本屏在整组详情页中的任务、全组视觉规范、产品一致性、接口参数和参考图边界。只输出优化后的完整提示词，不要解释，不要使用 Markdown。';
        let optimized = '';
    ...
    async function downloadAll(){
        ...
        try {
            const response = await fetch('/api/canvas-assets/download', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({filename:`${$('productName').value.trim() || '一键详情'}-素材.zip`, items})});
            if(!response.ok) throw new Error((await responseData(response)).detail || '下载失败');
            const blob = await response.blob();
            const href = URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = href;
            link.download = `${$('productName').value.trim() || '一键详情'}-素材.zip`;
            document.body.appendChild(link);
            link.click();
            link.remove();
            setTimeout(() => URL.revokeObjectURL(href), 1200);
    ...
    function normalizeRestoredGroup(item, index){
        if(item?.config && Array.isArray(item.cards)){
            const group = {
                ...
                title:item.title || `详情组 #${Number(item.serial) || index + 1}`,
                ...
                cards:item.cards.map((card, cardIndex) => ({
                    ...
                    title:card.title || `第 ${cardIndex + 1} 屏`,
                    prompt:card.prompt || '',
        ...
        return {
            id:uid(), serial, title:`详情组 #${serial}`,
    ...
    function restoreDraft(){
        ...
        if(!draft || typeof draft !== 'object'){
            syncCountDependentControls({count:8, modelUsage:4, reverseScreens:2});
            renderProducts();
            renderReferences();
            refreshGroups();
            return;
        }
        const adoptNewDefaults = Number(draft.schemaVersion) !== 3 && !(Array.isArray(draft.groups) && draft.groups.length);
        const restoredCount = adoptNewDefaults ? 8 : clampNumber(draft.count, 1, 30, 8);
        const restoredReverse = adoptNewDefaults ? 2 : clampNumber(draft.reverseScreens, 0, restoredCount, Math.min(2, restoredCount));
        const storedPrompt = String(draft.userPrompt || '');
        const promptRemainder = storedPrompt.replace(/^\s*输出\s*\d+\s*屏\s*[，,、]?\s*插入\s*反转屏\s*\d+\s*屏\s*[。.!！]?\s*/i, '').trimStart();
        $('productName').value = draft.productName || '';
        $('brandName').value = draft.brandName || '';
        $('sellingPoints').value = draft.sellingPoints || '';
        $('userPrompt').value = adoptNewDefaults
            ? `输出8屏，插入反转屏2屏。${promptRemainder ? `\n${promptRemainder}` : ''}`
            : storedPrompt || `输出${restoredCount}屏，插入反转屏${restoredReverse}屏。`;
        $('outputLanguage').value = draft.outputLanguage || '中文';
        if(!adoptNewDefaults && ['portrait916','wide','portrait','portrait43','square'].includes(draft.ratio)) $('ratioSelect').value = draft.ratio;
        else $('ratioSelect').value = 'portrait916';
        syncSizeOptions(adoptNewDefaults ? '' : draft.size || '');
        if(!adoptNewDefaults && ['low','medium','high'].includes(draft.quality)) $('qualitySelect').value = draft.quality;
        else $('qualitySelect').value = 'medium';
        if(restoredCount <= 12) $('countSelect').value = String(restoredCount);
        ...
        if(['auto','neutral-sans','humanist-sans','rounded','elegant-serif','modern-song','brush','tech','industrial','trend-hand'].includes(draft.fontStyle)) $('fontStyleSelect').value = draft.fontStyle;
        ...
        syncCountDependentControls({
            count:effectiveCount(),
            modelUsage:adoptNewDefaults ? Math.min(4, effectiveCount()) : clampNumber(draft.modelUsage, 0, effectiveCount(), Math.min(4, effectiveCount())),
            reverseScreens:restoredReverse
        });
        ...
        state.viewCount = !adoptNewDefaults && [4,5,6,8].includes(Number(draft.viewCount)) ? Number(draft.viewCount) : 6;
    ...
    function bind(){
        ...
        $('countSelect').onchange = handleCountChange;
        $('reverseScreens').onchange = handleReverseScreensChange;
        $('userPrompt').addEventListener('input', handleUserPromptInput);
        $('modelSettings').onchange = () => { syncCountDependentControls({count:effectiveCount()}); scheduleDraftSave(); };
        ['productName','brandName','sellingPoints','outputLanguage','sizeSelect','qualitySelect','copySettingSelect','fontStyleSelect','modelPose','modelUsage','fontProtection'].forEach(id => $(id)?.addEventListener('input', scheduleDraftSave));
        $('displayCount').onchange = event => { state.viewCount = Number(event.target.value) || 6; refreshGroups(); scheduleDraftSave(); };
        ...
        $('detailGroups').addEventListener('contextmenu', event => {
            const card = event.target.closest('.output-card[data-group-id][data-card-index]');
            if(!card) return;
            openCardContextMenu(event, findGroup(card.dataset.groupId), Number(card.dataset.cardIndex || 0));
        });
        ...
        $('collageZoomOut').onclick = () => setCollageZoom(state.collageFit ? .75 : state.collageZoom - .25);
        $('collageZoomIn').onclick = () => setCollageZoom(state.collageFit ? 1 : state.collageZoom + .25);
        $('collageFullscreen').onclick = () => void toggleCollageFullscreen();
        $('collageGrid').addEventListener('dblclick', event => {
            if(event.target.closest('.collage-sheet')) toggleCollageFit();
        });
        ...
        $('cardContextMenu').addEventListener('click', event => {
            ...
            const target = state.cardMenuTarget;
            if(!actionButton || !target || actionButton.disabled) return;
            const group = findGroup(target.groupId);
            const index = Number(target.cardIndex || 0);
            const action = actionButton.dataset.contextAction;
            closeCardContextMenu();
            if(action === 'prompt') openPromptEditor(group, index);
            if(action === 'retry') void rerunCard(group, index);
            if(action === 'download') downloadCardImage(group, index);
        });
        ...
        document.addEventListener('click', event => {
            if(!$('cardContextMenu').hidden && !event.target.closest('#cardContextMenu')) closeCardContextMenu();
        });
        document.addEventListener('scroll', closeCardContextMenu, true);
        document.addEventListener('fullscreenchange', syncCollageFullscreenButton);
        document.addEventListener('keydown', event => {
            if(event.key === 'Escape' && !$('cardContextMenu').hidden){ closeCardContextMenu(); return; }
