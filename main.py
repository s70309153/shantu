...
class DetailPageCollageRequest(BaseModel):
    urls: List[str] = []
    filename: str = "detail-page-collage.png"
    max_width: int = 2048
...
def effective_protocol(provider, model=""):
    ...
    if isinstance(overrides, dict):
        ...
        if val in PER_MODEL_PROTOCOL_OPTIONS:
            return val
    # 一些 OpenAI 兼容平台同时暴露 Gemini 图片模型，但不会把逐模型协议
    # 覆盖写入旧配置。模型名本身已经明确表达了原生 Gemini 图片协议，
    # 自动纠正这类旧配置；显式的 model_protocols 仍然拥有更高优先级。
    model_name = str(model or "").strip().lower()
    if base == "openai" and model_name.startswith("gemini-") and "image" in model_name:
        return "gemini"
    return base
...
def build_detail_page_collage(urls: List[str], max_width: int = 2048):
    images = []
    try:
        for raw_url in urls[:50]:
            ...
            try:
                if path and os.path.isfile(path):
                    ...
                    remote = fetch_remote_media_bytes(url, timeout=45.0, max_bytes=80 * 1024 * 1024)
                    if not remote:
                        continue
                    with Image.open(BytesIO(remote[0])) as source:
                        images.append(ImageOps.exif_transpose(source).copy())
            except Exception:
                continue
        images = [image for image in images if image.width > 0 and image.height > 0]
        if not images:
            raise HTTPException(status_code=404, detail="没有可拼接的图片")

        natural_width = max(image.width for image in images)
        width_limit = max(320, min(4096, int(max_width or 2048)))
        target_width = max(1, min(natural_width, width_limit))

        def scaled_heights(width):
            return [max(1, round(image.height * width / image.width)) for image in images]

        heights = scaled_heights(target_width)
        total_height = sum(heights)
        max_pixels = 80_000_000
        if target_width * total_height > max_pixels:
            factor = math.sqrt(max_pixels / float(target_width * total_height))
            target_width = max(320, int(target_width * factor))
            heights = scaled_heights(target_width)
            total_height = sum(heights)
        ...
        try:
            for image, height in zip(images, heights):
                resized = image.resize((target_width, height), Image.LANCZOS) if image.size != (target_width, height) else image
                if image_has_alpha(resized):
                    rgba = resized.convert("RGBA")
                    alpha = rgba.getchannel("A")
                    collage.paste(rgba, (0, offset_y), alpha)
                    alpha.close()
                    if rgba is not resized:
                        rgba.close()
                else:
                    rgb = resized.convert("RGB")
                    collage.paste(rgb, (0, offset_y))
                ...
                offset_y += height
            buffer = BytesIO()
            collage.save(buffer, format="PNG", compress_level=6)
            return buffer.getvalue(), target_width, total_height, len(images)
        finally:
            collage.close()
    finally:
        for image in images:
...
@app.post("/api/detail-page/collage")
async def create_detail_page_collage(payload: DetailPageCollageRequest, request: Request):
    ensure_same_origin_request(request)
    content, width, height, count = await asyncio.to_thread(build_detail_page_collage, payload.urls, payload.max_width)
    filename = sanitize_export_filename(payload.filename or "detail-page-collage.png", "detail-page-collage.png")
    ...
    headers = {
        ...
        "X-Collage-Width": str(width),
        "X-Collage-Height": str(height),
        "X-Collage-Count": str(count),
    }
